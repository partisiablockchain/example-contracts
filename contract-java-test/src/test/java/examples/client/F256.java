package examples.client;

import com.partisiablockchain.zk.real.protocol.binary.field.BinaryExtensionFieldElement;
import com.partisiablockchain.zk.real.protocol.field.FiniteFieldElement;
import java.util.List;
import java.util.stream.IntStream;

/** Represents an element in GF(2^8) using the reduction polynomial x^8+x^4+x^3+x+1. */
public final class F256 implements FiniteFieldElement<F256> {

  /** All valid elements of {@link BinaryExtensionFieldElement}. */
  private static final F256[] elements =
      IntStream.range(0, 256).mapToObj(i -> new F256((short) i)).toArray(F256[]::new);

  /** A hardcoded '0' for convenience. */
  public static final F256 ZERO = elements[0];

  /** A hardcoded '1' for convenience. */
  public static final F256 ONE = elements[1];

  private static final int[] LOOKUP_MULT_INVERSE =
      new int[] {
        ,
      };

  private final short value;

  private F256(short value) {
    this.value = value;
  }

  /**
   * Create element from byte.
   *
   * @param b element value
   * @return created element
   */
  public static F256 create(int b) {
    return elements[b & 0xff];
  }

  @Override
  public F256 add(F256 other) {
    return elements[this.value ^ other.value];
  }

  @Override
  public F256 subtract(F256 other) {
    return elements[this.value ^ other.value];
  }

  @Override
  public F256 multiply(F256 other) {
    int a = this.value;
    int b = other.value;
    int m;
    int p = 0;
    for (int i = 0; i < 8; i++) {
      p = (p ^ (-(b & 1) & a)) & 0xff;
      m = -((a >> 7) & 1) & 0xff;
      a = ((a << 1) ^ (0b100011011 & m)) & 0xff;
      b = b >> 1;
    }
    return elements[p];
  }

  @Override
  public F256 negate() {
    return this;
  }

  @Override
  public F256 modInverse() {
    return elements[LOOKUP_MULT_INVERSE[this.value]];
  }

  @Override
  public F256 squareRoot() {
    throw new UnsupportedOperationException("Square root is not supported for this type");
  }

  @Override
  public boolean isZero() {
    return this.value == 0;
  }

  @Override
  public boolean isOne() {
    return this.value == 1;
  }

  @Override
  public byte[] serialize() {
    return new byte[] {(byte) this.value};
  }

  @Override
  public String toString() {
    return "%X".formatted(value);
  }

  /**
   * Get the alphas for interpolating polynomials over this field.
   *
   * @param numberNodes the number of nodes to generate alphas for
   * @return the alphas
   */
  public static List<F256> alphas(int numberNodes) {
    return IntStream.range(1, numberNodes + 1).mapToObj(F256::create).toList();
  }

  /**
   * Get the integral value.
   *
   * @return integral value
   */
  public int intValue() {
    return value;
  }
}
